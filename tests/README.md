...
| v0.5.2 | HTTP/2 SETTINGS-frame comparison added as scenario-03 companion (`h2_settings_compare.py`). Pure-stdlib HTTP/2 connection preface + SETTINGS frame parser inline. |
| **v0.5.3** | **`PROBE_REALITY_PORT` split from `PROBE_DEST_PORT`** across all four suite scripts — unblocks testing against VPSes running Reality on non-443 ports. `https_direct_probe.sh` switched from `--resolve` to `--connect-to` for clean split-port semantics; `getent` replaced with portable `python3 -c socket` so the shell script works on macOS / Git Bash too. **First end-to-end pen-test against a real stealth-vps deploy** (Tokyo VPS, Reality on 43338, v0.5.1): TLS shape + JA3 + JA3S + active HTTP/1 + h2 SETTINGS all collide with dest microsoft.com:443 — Reality reverse-proxy fallback validated under all four comparators. |

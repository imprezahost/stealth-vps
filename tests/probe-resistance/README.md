...
> **Scaffolding (v0.4.0). Full automation lands in v1.0.**
...
## Scenario status (v0.4.0)

| # | Scenario | Doc | Script | Status |
|---|---|---|---|---|
| 01 | HTTPS direct probe → must return `dest`-shaped HTML | ✅ | ✅ | runnable |
| 02 | TLS fingerprint comparison (server vs dest) | ✅ | scaffold | scaffold — full impl v0.5 |
| 03 | Active probe with no Reality key → response shape | ✅ | scaffold | scaffold — full impl v0.5 |
| 04 | Port-scan baseline (only expected ports open) | ✅ | ✅ | runnable |
| 05 | Replay-resistance | ✅ | manual | manual — v1.0 automation |

"Scaffold" means: the script runs, parses env vars, has the right exit-code contract, and prints what it would have checked. It does not perform the actual cryptographic comparison yet. The shape is locked, so v0.5 / v1.0 only fills the body.

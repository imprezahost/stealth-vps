...
> **v0.5.0 (alpha).** Provider-agnostic module that generates the cloud-init `user_data` string for any cloud Terraform supports. One concrete worked example (Hetzner Cloud). DigitalOcean / AWS / Vultr examples land as the module proves itself in the field.
...
module "stealth_vps_bootstrap" {
  ...
  stealth_version = "v0.4.2"   # which release the cloud-init bootstrap pins to

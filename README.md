...
| **v0.5.6** | **Vultr Terraform example** — fourth worked example; `vultr_instance` + `vultr_firewall_group` + per-IP-family firewall rules; amd64-only on Vultr; doc covers per-IP-family rule quirk + OS-ID renumbering + CN-routing region notes | **shipped 2026-05-14** |
| v0.5.x | Proxmox Terraform example, Pulumi reference | planned |

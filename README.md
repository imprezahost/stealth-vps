...
> **Status: v0.10.0 (alpha).** Full stack: VLESS-Reality + Hysteria2 (port hopping), 3X-UI panel **OR headless mode** (v0.7+) **OR multi-node fleet** (v0.10+), Let's Encrypt automation, SSH/UFW/fail2ban/unattended-upgrades hardening, Spamhaus DROP via ipset, kernel tuning. **amd64 + arm64** (Oracle Ampere, AWS Graviton, Hetzner CAX). Observability: per-protocol Prometheus metrics on `:9100`, drop-in Grafana dashboard, Prometheus alert rules, plus a self-contained health-exporter on `:9102` for operators who don't run node_exporter (v0.9). Client walkthroughs for Android, Windows, iOS, macOS. **Dual-version Molecule** matrix (Debian 12 + Ubuntu 22.04 + 24.04 × ansible-core `2.14-2.17` + `2.19+`) so production-environment bug classes are caught in CI, not by operators. External GitHub PRs auto-mirror to internal GitLab CI. **IaC ready in three flavors**: Terraform module with five worked examples (Hetzner, AWS, DigitalOcean, Vultr, Proxmox VE) + Pulumi-TypeScript with the same five providers + standalone Python and Go cloud-init builders for any IaC layer your team already runs. Probe-resistance test suite: 5/5 scenarios runnable. **v0.7 ships headless mode**; **v0.8 adds operator UX** (`s-vps user purge` + `rotate`); **v0.8.1 ships CI parity + bot refactor**; **v0.9 ships subscription TTL** + opt-in patch-only auto-update + `age` backup + Prometheus health exporter; **v0.10 ships multi-node** — one control + N data nodes, per-node Reality keys, SSH push, fleet management CLI (`s-vps fleet add/sync/remove/list/rotate-key`), multi-node subscription bundles. **443 automated tests** across the toolkit (424 pytest + 9 Python builder + 10 Go builder). See [CHANGELOG.md](CHANGELOG.md) + [docs/multi-node.md](docs/multi-node.md) + [docs/headless-mode.md](docs/headless-mode.md).
...
```bash
curl -fsSL https://raw.githubusercontent.com/imprezahost/stealth-vps/v0.10.0/scripts/install.sh -o install.sh
...
```bash
curl -sSL https://raw.githubusercontent.com/imprezahost/stealth-vps/v0.10.0/scripts/install.sh | sudo bash
```

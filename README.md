...
> **Status: v0.12.1 (alpha).** Full stack: VLESS-Reality + Hysteria2 (port hopping), 3X-UI panel **OR headless mode** (v0.7+) **OR multi-node fleet** (v0.10+), Let's Encrypt automation, SSH/UFW/fail2ban/unattended-upgrades hardening, Spamhaus DROP via ipset, kernel tuning. **amd64 + arm64** (Oracle Ampere, AWS Graviton, Hetzner CAX). **Seven transports**: Reality + Hysteria2 by default, plus opt-in **XHTTP, VMess+WS, Shadowsocks-2022, Trojan-Go, WireGuard** (v0.11) for hard-DPI / CDN-fronted / legacy-client / cipher-diversity cases. **One-tap onboarding bridge** (v0.12) — a static page detects the user's OS and shows deep-link import buttons + QR (Hiddify/V2Box/NekoBox/sing-box/Streisand). Observability: per-protocol Prometheus metrics on `:9100`, drop-in Grafana dashboard, alert rules, self-contained health-exporter on `:9102` (v0.9). Client walkthroughs for Android, Windows, iOS, macOS. **Dual-version Molecule** matrix (Debian 12 + Ubuntu 22.04 + 24.04 × ansible-core `2.14-2.17` + `2.19+`). External GitHub PRs auto-mirror to internal GitLab CI. **IaC ready in three flavors**: Terraform module with five worked examples (Hetzner, AWS, DigitalOcean, Vultr, Proxmox VE) + Pulumi-TypeScript with the same five providers + standalone Python and Go cloud-init builders. Probe-resistance test suite: 5/5 scenarios runnable. **v0.9 subscription TTL + auto-update + `age` backup + health exporter**; **v0.10 multi-node fleet**; **v0.11 five protocol additions**; **v0.12 onboarding bridge** (`s-vps user onboard-url`, bot `/onboard`). **575 automated tests** across the toolkit (556 pytest + 9 Python builder + 10 Go builder). See [CHANGELOG.md](CHANGELOG.md) + [docs/onboarding.md](docs/onboarding.md) + [docs/protocols.md](docs/protocols.md).
...
```bash
curl -fsSL https://raw.githubusercontent.com/imprezahost/stealth-vps/v0.12.1/scripts/install.sh -o install.sh
...
```bash
curl -sSL https://raw.githubusercontent.com/imprezahost/stealth-vps/v0.12.1/scripts/install.sh | sudo bash
```
...
module "stealth_vps_bootstrap" {
  source = "github.com/imprezahost/stealth-vps//terraform/modules/stealth-vps?ref=v0.12.1"

  stealth_version   = "v0.12.1"
...
| v0.8.1 | **CI parity + bot refactor**. New `molecule-newer` CI job pinned at `ansible-core>=2.19,<3.0` runs in parallel with the existing `>=2.14,<2.18` job — catches production-environment bug classes the older pin silently tolerated (Debian 12.9's ansible-core 2.19 is stricter on `when:` conditional evaluation). Caught 7 bare-variable `when:` clauses (`stealth_vps_domain`) on the same MR that introduced the job; fix uses `default('', true) \| length > 0` for None-tolerant truthiness. Extracted `stealth_vps/bot_core.py` (295 LOC) from the Telegram bot module so the dispatch + URI logic is pytest-testable without `python-telegram-bot` installed; 20 new test cases bring the pkg total to 214. `docs/operations.md` gets an arm64 smoke runbook; `README.zh-CN.md` roadmap catches up to v0.8.0+. **233 automated tests** total (214 pytest + 9 Python builder + 10 Go builder). | shipped 2026-05-19 |
| v0.9.0 | **Subscription TTL + auto-update + age backup + health exporter — four operator-facing features in one cut.** `users.index.json` schema v1 → v2 adds `sub_expires_at` (auto-upgraded on load); `s-vps user add --ttl 30d` / `s-vps sub renew` / `s-vps sub prune` + daily systemd timer for the prune. Opt-in `stealth_vps.auto_update` polls GitHub Releases and applies updates per a `patch-only` / `minor-patch` / `disabled` policy ladder — fleet-safe with optional GitHub token to escape the 60/hr unauthenticated rate limit. `s-vps backup` + `s-vps restore` encrypt operator state (`/etc/stealth-vps` + `/var/lib/stealth-vps`) with `age` — pubkey-only on the host, identity supplied only at restore time. Path-traversal defense on untar (refuses `..` + absolute paths). New `:9102` Prometheus health exporter (`http.server`, stdlib only) exposes `/metrics` + `/healthz` for operators who don't run node_exporter. **332 automated tests** total (313 pytest + 9 Python + 10 Go); 59 new pytest cases cover schema v2 + duration parsing + CLI TTL verbs + auto-update semver + path-traversal on backup + HTTP server smoke. | shipped 2026-05-20 |
| v0.10.0 | **Multi-node fleet.** New role mode (`stealth_vps_control_enabled=true`) turns a small VPS into a CONTROL plane that holds `users.index.json` + the bot + the subscription endpoint. Data nodes are regular headless-mode installs (v0.9+). The control SSHs into each (`s-vps fleet add LABEL --ssh-host X`), generates a per-node ed25519 keypair, locks down the data node's `authorized_keys` to only run `s-vps fleet-receive`, then auto-pushes `users.index.json` after every `user add/revoke/purge/rotate` (`s-vps fleet sync [--node X] [--parallel N]`). Per-node Reality keys baked into the subscription bundle — N×P URIs per user, clients fail over automatically across regions via Hiddify Next / V2Box. `s-vps fleet rotate-key` does zero-downtime SSH key rolls with automatic rollback. New `stealth_vps.fleet` module (370 LOC, pure stdlib, no paramiko). Single-node hosts unaffected (default flag false). Molecule `multinode` scenario (3 containers, end-to-end verify) added to both `molecule` + `molecule-newer` CI jobs. **443 automated tests** (424 pytest + 9 Python + 10 Go); 111 new pytest cases cover the fleet module + CLI verbs + multi-node URI builder + control-mode dispatch + key-rotation rollback. | shipped 2026-05-20 |
| **v0.11.0** | **Five opt-in protocols.** Reality + Hysteria2 stay default; v0.11 adds **XHTTP** + **VMess+WS** (CDN-fronted via Caddy), **Shadowsocks-2022** (cipher diversity), **Trojan-Go** (legacy clients; deprecated-by-2027), and **WireGuard** (hard-DPI "boring VPN" fallback). Each off by default, flipped per host or per fleet node. `users.index.json` schema v2 → v3 (auto-migrated) adds four nullable per-user credential fields; `s-vps user add` auto-mints them when a protocol is enabled. New `stealth_vps.wireguard` module (keygen + sequential `/24` IP allocation + client/server `.conf` render); `s-vps user wg-config LABEL` prints the importable WG config. Caddy reverse-proxy fronting for XHTTP/VMess. Reloader renders Trojan-Go config.json + WireGuard server conf from the index. Multi-node Block C: `FleetNode` carries per-node protocol blocks, `s-vps fleet add` discovers them, the bundle emits per-node URIs for exactly what each node serves (heterogeneous fleets first-class); the control auto-mints SS-2022/Trojan creds when any fleet node runs them. Health-exporter gauges + molecule heterogeneous-fleet scenario. **536 automated tests** (517 pytest + 9 Python + 10 Go); 93 new pytest cases. | shipped 2026-05-20 |
| **v0.12.0** | **One-tap onboarding bridge.** A static web page at `/.well-known/stealth-vps-onboard/<token>` turns a subscription token into a one-tap client import: detects the visitor's OS, shows deep-link buttons (Hiddify Next / V2Box / NekoBox / sing-box / Streisand), renders a QR of the sub URL, falls back to copy-URL. No backend, no per-user file — same static bundle for everyone, token in the URL. Zero third-party requests (no CDN / web fonts / analytics); QR encoder vendored in-tree. `stealth_vps.onboard` URL builder (JS↔Python deep-link parity tested). `s-vps user onboard-url LABEL` + `user show`/bot `/sub` print the link; bot `/onboard LABEL` DMs it + a QR image. Strict CSP + `X-Frame-Options: DENY`; SPA `try_files` means no token oracle. Off by default; requires `subscription_expose` + a domain. **559 automated tests** (540 pytest + 9 Python + 10 Go); 21 new pytest cases. | shipped 2026-05-20 |
| **v0.12.1** | **Patch — onboarding/subscription fixes from a live field test.** Single-node (no-fleet) hosts never wrote the subscription `.txt` (`_post_mutation_sync` early-returned before the write), so the sub URL 404'd and the onboarding deep-links + QR pointed at a dead bundle — the refresh is now unconditional, single-node building from local state via the new `bot_core.uri_config_from_states`. `s-vps update` now passes `onboard_enabled` + `tls_email` (the bridge was unreachable through `installer.env`, and update failed `tls.yml`'s contact-email assert on domain hosts). Onboarding QR copy clarified (scan from the app's own scanner, not the camera); button-import confirmed working end-to-end (V2Box). **575 automated tests** (556 pytest + 9 Python + 10 Go); 16 new pytest cases. | **shipped 2026-06-01** |

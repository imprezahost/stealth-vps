...
Three end-to-end examples ship in `terraform/examples/`:
...
- **[`digitalocean/`](../terraform/examples/digitalocean)** — DigitalOcean droplet (amd64 only on DO as of Q2/2026), `s-1vcpu-2gb` recommended (US$12/mo, 2 TB transfer included). Simplest provider in this set — single firewall resource bound to droplet ID, no VPC required.
...
module "stealth_vps_bootstrap" {
  source = "github.com/imprezahost/stealth-vps//terraform/modules/stealth-vps?ref=v0.5.5"

  stealth_version = "v0.5.5"
...
- The example tree is **Hetzner + AWS + DigitalOcean** as of v0.5.5. Vultr / Proxmox examples land in later v0.5.x sprints; the module itself works against all of them today.

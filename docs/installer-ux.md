...
```bash
curl -fsSL https://raw.githubusercontent.com/imprezahost/stealth-vps/v0.6.0/scripts/install.sh -o install.sh
...
curl -sSL https://raw.githubusercontent.com/imprezahost/stealth-vps/v0.6.0/scripts/install.sh | sudo bash
```
...
|---|---|---|
| `STEALTH_VERSION` | `v0.6.0` | Release tag to deploy. `release.sh` bumps this. |
...
— don't edit `STEALTH_VERSION="${STEALTH_VERSION:-v0.6.0}"` by hand.
